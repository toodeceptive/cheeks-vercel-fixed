...
| PATCH-004 | Agent 0 | QR code PDF | Add source parameter to QR code URL | Current URL lacks source tracking parameter | `agent5.nonrepo.md:68-71` (archived), `AUDIT_FINDINGS.md:FINDING-HIGH-002` | 1. Open QR code PDF<br>2. Update URL to include `?src=qr-table-tent`<br>3. Regenerate/update QR code<br>4. Test source tracking | LOW | Revert QR code if needed |
| PATCH-005 | Agent 0 | schema.json, index.html | Add Google Business Profile URL | Missing Google Business Profile reference in listings | `agent4.web.md` (archived), `AUDIT_FINDINGS.md:FINDING-HIGH-003` | 1. Obtain Google Business Profile URL<br>2. Add to `sameAs` array in schema.json<br>3. Add to `sameAs` in index.html JSON-LD<br>4. Verify on live site | LOW | Remove URL if incorrect |
...
| PATCH-007 | Agent 0 | assets/images/ | Add missing staff image | HTML references staff.webp but file missing | `agent1.local.md` (archived), `AUDIT_FINDINGS.md:FINDING-MED-003` | 1. Add staff.webp image to assets/images/<br>2. OR remove reference from HTML<br>3. Verify fallback handling works | LOW | Remove image or reference if needed |
